...
class Order extends CI_Controller {
    ...
    public function view($id = null){
        ...
        if(!$id || !$data['products']){
            $this->session->set_flashdata('mensage', array('danger','Pedido no encontrado!'));
            redirect('/');
        }
        $order_price = 0;
    ...
    public function save($id = null)
    {
        ...
        if($send_form){
            $this->session->set_flashdata('mensagem', array('success','Pedido agregado con exito!'));
            redirect('');
        }
        else
        {
            $this->session->set_flashdata('mensagem', array('danger','Seleccione minimo un prodicto!'));
            redirect('');
        }
        $products = true;
    }
    ...
    public function delete($id)
    {
        ...
        if($delete){
            $this->session->set_flashdata('mensagem', array('success','Pedido eliminado con exito!'));
            redirect('order');
        }
        else
        {
            $this->session->set_flashdata('mensagem', array('danger','Ops! Pedido no encontrado!'));
            redirect('order');
        }
    }
}
